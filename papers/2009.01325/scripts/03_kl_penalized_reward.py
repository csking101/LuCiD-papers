...
#
# Run:
#   manim -ql --media_dir ../output/animations 03_kl_penalized_reward.py KLPenalizedReward
...
class KLPenalizedReward(Scene):
    def construct(self):
        ...
        # ── Title ──
        title = Text("The KL-Penalized Reward", font_size=34,
        ...
        # ══════════════════════════════════════════════════════════════
        box1 = SurroundingRectangle(full_eq[2], color="#f0883e", buff=0.1,
                                     stroke_width=2, corner_radius=0.05)
        label1 = Text("Reward Model Score", font_size=20, color="#f0883e",
                       weight=BOLD, font=_FONT)
        label1.next_to(box1, DOWN, buff=0.3)

        desc1 = VGroup(
            Text("• Learned from 65K human comparisons", font_size=16,
                 color="#c9d1d9", font=_FONT),
            Text("• Higher = more preferred by humans", font_size=16,
                 color="#c9d1d9", font=_FONT),
            Text("• The \"carrot\" — what we want to maximize", font_size=16,
                 color="#f0883e", font=_FONT),
        ).arrange(DOWN, buff=0.2, aligned_edge=LEFT)
        desc1.next_to(label1, DOWN, buff=0.3)
        ...
        box2 = SurroundingRectangle(kl_group, color="#d2a8ff", buff=0.1,
                                     stroke_width=2, corner_radius=0.05)
        label2 = Text("KL Divergence Penalty", font_size=20, color="#d2a8ff",
                       weight=BOLD, font=_FONT)
        label2.next_to(box2, DOWN, buff=0.3)
        ...
        kl_expanded.next_to(label2, DOWN, buff=0.3)
        ...
        # ══════════════════════════════════════════════════════════════
        p1_title = Text("Purpose 1: Entropy Bonus", font_size=22,
        ...
        p1_desc = VGroup(
            Text("Encourages exploration — prevents mode collapse",
                 font_size=17, color="#c9d1d9", font=_FONT),
            Text("Without it: policy collapses to a single \"best\" output",
                 font_size=17, color="#E74C3C", font=_FONT),
            Text("With it: policy maintains diversity in generations",
                 font_size=17, color="#3fb950", font=_FONT),
        ).arrange(DOWN, buff=0.25, aligned_edge=LEFT)
        p1_desc.next_to(p1_title, DOWN, buff=0.3)
        ...
        # ══════════════════════════════════════════════════════════════
        p2_title = Text("Purpose 2: Stay Near SFT Distribution", font_size=22,
                         color="#58a6ff", weight=BOLD, font=_FONT)
        ...
        p2_desc.next_to(p2_title, DOWN, buff=0.3)
