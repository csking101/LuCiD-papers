...
class PolicyGradientIntuition(Scene):
    def construct(self):
        ...
        # ── Title ──
        title = Text("Policy Gradient: The Intuition", font_size=36, weight=BOLD, color=WHITE)
        ...
        # STEP 1: The Setup
        # ════════════════════════════════════════
        step1 = Text("Step 1: The Agent and Environment", font_size=28, weight=BOLD, color="#58a6ff")
        ...
        agent_label = Text("Agent", font_size=20, weight=BOLD, color="#58a6ff")
        agent_label.move_to(agent_box)
        agent = VGroup(agent_box, agent_label).move_to(LEFT * 3)

        # Policy inside agent
        policy_label = Text("Policy: pi(a|s)", font_size=14, color=GREY)
        ...
        env_label = Text("Environment", font_size=20, weight=BOLD, color="#3fb950")
        ...
        state_text = Text("state s", font_size=14, color="#3fb950")
        ...
        action_text = Text("action a", font_size=14, color="#58a6ff")
        ...
        # Reward
        reward_text = Text("reward r", font_size=14, color="#f0883e")
        ...
        # ════════════════════════════════════════
        step2 = Text("Step 2: Policy Outputs Action Probabilities", font_size=28, weight=BOLD, color="#58a6ff")
        ...
        state_lab = Text("State: s_t", font_size=18, color="#3fb950")
        ...
        nn_label = Text("Neural Net", font_size=16, weight=BOLD, color="#d2a8ff")
        nn_theta = Text("(theta)", font_size=14, color=GREY)
        ...
        for i, (act, p, c) in enumerate(zip(actions, probs, colors_act)):
            ...
            lab = Text(act, font_size=14, color=c)
            lab.next_to(bar, DOWN, buff=0.1)
            bar_labels.add(lab)

            plab = Text(f"{p:.0%}", font_size=14, color=WHITE)
        ...
        sampled = Text("Sampled!", font_size=16, weight=BOLD, color="#3fb950")
        ...
        # ════════════════════════════════════════
        step3 = Text("Step 3: Reward Adjusts Future Probabilities", font_size=28, weight=BOLD, color="#58a6ff")
        step3.to_edge(UP, buff=0.4)
        self.play(Write(step3), run_time=0.6)

        # Good reward scenario
        good_label = Text("Good reward (A > 0): increase P(Right)", font_size=20, color="#3fb950")
        good_label.next_to(step3, DOWN, buff=0.6)
        self.play(FadeIn(good_label))

        # Before bars
        before_label = Text("Before", font_size=16, color=GREY)
        ...
        for i, (p, c) in enumerate(zip(probs_before, colors_act)):
            ...
            bars_before.add(bar)
            lab = Text(f"{p:.0%}", font_size=12, color=WHITE)
            lab.next_to(bar, UP, buff=0.05)
            bars_before.add(lab)

        # After bars (good reward)
        after_label = Text("After", font_size=16, color=GREY)
        ...
        update_arrow = Arrow(LEFT * 2.5 + DOWN * 0.5, RIGHT * 0.5 + DOWN * 0.5,
                              color="#3fb950", buff=0.1, stroke_width=3)
        update_text = Text("gradient ascent", font_size=14, color="#3fb950")
        ...
        bad_label = Text("Bad reward (A < 0): decrease P(Right)", font_size=20, color="#E74C3C")
        bad_label.next_to(step3, DOWN, buff=0.6)
        ...
        before_label2 = Text("Before", font_size=16, color=GREY)
        ...
        after_label2 = Text("After", font_size=16, color=GREY)
        ...
        update_text2 = Text("gradient ascent", font_size=14, color="#E74C3C")
        ...
        # ════════════════════════════════════════
        step4 = Text("The REINFORCE Loop", font_size=28, weight=BOLD, color="#58a6ff")
        ...
        steps = [
            ...
            ("3", "Compute gradient: reward * grad(log pi(a|s))", "#d2a8ff"),
        ...
        for num, text, color in steps:
            row = VGroup(
                Text(num + ".", font_size=20, weight=BOLD, color=color),
                Text(text, font_size=18, color=WHITE),
