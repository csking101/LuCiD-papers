...
#
# Run:
#   manim -ql --media_dir ../output/animations 09_ppo_vs_trpo_vs_pg.py PPOvsTRPOvsPG
...
def _copy_to_docs():
    src = _PAPER_DIR / "output/animations/videos/09_ppo_vs_trpo_vs_pg/480p15/PPOvsTRPOvsPG.mp4"
...
class PPOvsTRPOvsPG(Scene):
    def construct(self):
        ...
        # ── Title ──
        title = Text("PPO vs TRPO vs Vanilla Policy Gradient", font_size=32, weight=BOLD, color=WHITE)
        subtitle = Text("Three approaches to the same problem", font_size=20, color=GREY)
        ...
        # ════════════════════════════════════════
        header = Text("Training Performance Over Time", font_size=26, weight=BOLD, color="#58a6ff")
        ...
        for i, (name, color) in enumerate(methods):
            ...
            label = Text(name, font_size=18, weight=BOLD, color=color)
            label.next_to(ax, UP, buff=0.15)
        ...
        for ax in axes_group:
            xl = Text("Updates", font_size=10, color=GREY)
            xl.next_to(ax.x_axis, DOWN, buff=0.1)
            yl = Text("Reward", font_size=10, color=GREY)
            yl.next_to(ax.y_axis, LEFT, buff=0.1)
            self.add(xl, yl)
        ...
        # ═══ Add annotations ═══
        vanilla_note = Text("Unstable, collapses", font_size=12, color="#E74C3C")
        vanilla_note.next_to(axes_group[0], DOWN, buff=0.4)

        trpo_note = Text("Stable, but slow", font_size=12, color="#d2a8ff")
        trpo_note.next_to(axes_group[1], DOWN, buff=0.4)

        ppo_note = Text("Stable AND fast", font_size=12, color="#3fb950")
        ...
        # WHY PPO WINS
        # ════════════════════════════════════════
        why_label = Text("Why PPO Wins", font_size=28, weight=BOLD, color="#3fb950")
        why_label.to_edge(UP, buff=0.4)
        self.play(Write(why_label), run_time=0.5)

        # Comparison table
        headers = VGroup(
            Text("Method", font_size=16, weight=BOLD, color=GREY),
            Text("Step Control", font_size=16, weight=BOLD, color=GREY),
            Text("Complexity", font_size=16, weight=BOLD, color=GREY),
            Text("Result", font_size=16, weight=BOLD, color=GREY),
        ...
        for idx, (method, step_ctrl, complexity, result) in enumerate(rows):
            row = VGroup(
                Text(method, font_size=15, color=row_colors[idx], weight=BOLD),
                Text(step_ctrl, font_size=15, color=WHITE),
                Text(complexity, font_size=15, color=WHITE),
                Text(result, font_size=15, color=row_colors[idx]),
            ).arrange(RIGHT, buff=1.0)
            table_rows.add(row)
