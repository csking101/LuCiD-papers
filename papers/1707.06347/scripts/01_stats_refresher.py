...
class StatsRefresher(Scene):
    def construct(self):
        ...
        # ── Title ──
        title = Text("Statistics Refresher for PPO", font_size=36, weight=BOLD, color=WHITE)
        subtitle = Text("Four key concepts you need", font_size=22, color=GREY)
        ...
        # 1. EXPECTATION
        # ════════════════════════════════════════
        sec1 = Text("1. Expectation", font_size=32, weight=BOLD, color="#58a6ff")
        sec1.to_edge(UP, buff=0.5)
        self.play(Write(sec1), run_time=0.7)

        # Concept: weighted average
        eq_title = Text("The weighted average of outcomes", font_size=20, color=GREY)
        eq_title.next_to(sec1, DOWN, buff=0.4)
        self.play(FadeIn(eq_title))

        # Show discrete case
        disc = Text("Discrete:  E[f(x)] = sum of  p(x) * f(x)", font_size=20, color=WHITE)
        disc.next_to(eq_title, DOWN, buff=0.5)
        self.play(Write(disc), run_time=0.8)
        self.wait(0.5)

        # Show continuous case
        cont = Text("Continuous:  E[f(x)] = integral of  p(x) * f(x) dx", font_size=20, color=WHITE)
        cont.next_to(disc, DOWN, buff=0.3)
        self.play(Write(cont), run_time=0.8)
        self.wait(0.5)

        # Key insight
        key1 = Text("In RL: we can't compute the integral,", font_size=18, color="#f0883e")
        key1b = Text("so we estimate by sampling trajectories", font_size=18, color="#f0883e")
        key1.next_to(cont, DOWN, buff=0.5)
        key1b.next_to(key1, DOWN, buff=0.15)
        self.play(FadeIn(key1), FadeIn(key1b))

        # Sampling visual: dots appearing
        sample_label = Text("Sample average approximates expectation:", font_size=18, color=GREY)
        sample_label.next_to(key1b, DOWN, buff=0.4)
        self.play(FadeIn(sample_label))

        approx = Text("E[f(x)]  ~  (1/N) * sum of f(x_i)", font_size=20, color="#3fb950")
        ...
        # 2. VARIANCE
        # ════════════════════════════════════════
        sec2 = Text("2. Variance", font_size=32, weight=BOLD, color="#58a6ff")
        sec2.to_edge(UP, buff=0.5)
        self.play(Write(sec2), run_time=0.7)

        var_def = Text("Var[X] = E[(X - E[X])^2]", font_size=22, color=WHITE)
        var_def.next_to(sec2, DOWN, buff=0.5)
        self.play(Write(var_def), run_time=0.8)

        meaning = Text("How spread out are our estimates?", font_size=20, color=GREY)
        meaning.next_to(var_def, DOWN, buff=0.4)
        self.play(FadeIn(meaning))
        self.wait(0.5)

        # Low variance visual
        low_var_label = Text("Low Variance: estimates cluster tightly", font_size=18, color="#3fb950")
        ...
        high_var_label = Text("High Variance: estimates all over", font_size=18, color="#E74C3C")
        ...
        rl_note = Text("In RL: high variance gradients = unstable training", font_size=18, color="#f0883e")
        ...
        # ════════════════════════════════════════
        sec3 = Text("3. Importance Sampling", font_size=32, weight=BOLD, color="#58a6ff")
        sec3.to_edge(UP, buff=0.5)
        self.play(Write(sec3), run_time=0.7)

        problem = Text("Problem: we have samples from p(x),", font_size=20, color=WHITE)
        problem2 = Text("but want to evaluate E under q(x)", font_size=20, color=WHITE)
        ...
        # The trick
        trick = Text("E_q[f(x)] = E_p[ (q(x)/p(x)) * f(x) ]", font_size=22, color="#3fb950")
        trick.next_to(problem2, DOWN, buff=0.5)
        self.play(Write(trick), run_time=0.8)

        ratio_label = Text("q(x)/p(x) = importance weight", font_size=18, color="#d2a8ff")
        ratio_label.next_to(trick, DOWN, buff=0.3)
        self.play(FadeIn(ratio_label))
        self.wait(0.5)

        # PPO connection
        ppo_note = Text("In PPO: data from old policy, evaluate new policy", font_size=18, color="#f0883e")
        ppo_note2 = Text("r(t) = pi_new(a|s) / pi_old(a|s)", font_size=20, color="#f0883e")
        ppo_note.next_to(ratio_label, DOWN, buff=0.5)
        ppo_note2.next_to(ppo_note, DOWN, buff=0.2)
        self.play(FadeIn(ppo_note), run_time=0.5)
        self.play(Write(ppo_note2), run_time=0.7)

        warn = Text("Caveat: if p and q are very different, weights blow up", font_size=16, color="#E74C3C")
        ...
        # 4. KL DIVERGENCE
        # ════════════════════════════════════════
        sec4 = Text("4. KL Divergence", font_size=32, weight=BOLD, color="#58a6ff")
        sec4.to_edge(UP, buff=0.5)
        self.play(Write(sec4), run_time=0.7)

        kl_def = Text("KL[p || q] = sum of  p(x) * log(p(x)/q(x))", font_size=20, color=WHITE)
        kl_def.next_to(sec4, DOWN, buff=0.5)
        self.play(Write(kl_def), run_time=0.8)

        meaning_kl = Text("Measures how different two distributions are", font_size=20, color=GREY)
        ...
        # Properties
        props = VGroup(
            Text("KL >= 0  always", font_size=18, color=WHITE),
            Text("KL = 0   only when p = q", font_size=18, color=WHITE),
            Text("KL is NOT symmetric:  KL[p||q] != KL[q||p]", font_size=18, color=WHITE),
        ...
        dist_label = Text("Small KL: distributions are similar", font_size=18, color="#3fb950")
        dist_label2 = Text("Large KL: distributions are very different", font_size=18, color="#E74C3C")
        ...
        ppo_kl = Text("In PPO: constrain KL to keep policy updates safe", font_size=18, color="#f0883e")
        ...
        # ── Summary ──
        summary_title = Text("Summary", font_size=32, weight=BOLD, color="#58a6ff")
        summary_title.to_edge(UP, buff=0.5)
        self.play(Write(summary_title), run_time=0.5)

        items = VGroup(
            Text("1. Expectation  -  estimate via sampling", font_size=20, color=WHITE),
            Text("2. Variance  -  lower is better for gradients", font_size=20, color=WHITE),
            Text("3. Importance Sampling  -  reuse old data for new policy", font_size=20, color=WHITE),
            Text("4. KL Divergence  -  measure policy change", font_size=20, color=WHITE),
